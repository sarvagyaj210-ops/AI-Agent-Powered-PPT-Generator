...
# =============FRONT END=============
st.title("AGENT-POWERED PPT GENERATOR")
...
def search_latest_info(query):
  """This Function helps to get latest news or content from website using tavily,
...
def generate_image(img_prompt):
  """This Fuction helps to generate image using free API with given img_prompt using pollinations"""
...
def prompt_generator(model, query):
  prompt = f""" Your task is to give retained prompt instructions for given

  prompt:
  You are a professional PPT generator where user will give the query and based
  on that you have to generate dynamic HTML output waste PPT with advanced CSS
  and dynamic UI and UX with PPT toggle button based on query take image
  reference to generate And embed the same in PPT using
...
if all(ALL_API) and user_query:
  ...
  with tab2:
    ...
    if st.button("Fetch news: ", key = "news_button"):
      with st.spinner("Running Agent..."):
        prompt = """ Give latest news India or world news rated to tech, business, jobs 
        or user requested Output in proper HTML News Templates""" + user_query
